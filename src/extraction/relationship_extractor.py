        ...
        if entities:
            ...
            if self._llm_cfg and self._domain:
                if self._prompt_store:
                    system_prompt, user_layout = self._prompt_store.load_relationship_prompts(
                        model_name=self._model_name or "_default",
                        domain_name=self._domain_name,
                        llm_cfg=self._llm_cfg,
                        domain=self._domain,
                    )
                    user_prompt = user_layout.with_text_and_entities(text, entity_list)
                else:
                    system_prompt, user_template = build_relationship_prompts(
                        self._llm_cfg, self._domain,
                    )
